...
class FilesTest < Test::Unit::TestCase
  ...
  def test_creating_file
    post('/api?token=token', 'media' => zip)
    assert_ok
    item = App::Item.first
    assert(!item.nil?)
  ...
  def test_creating_file_custom_slug
    post('/api?token=token&slug=test', 'media' => zip)
    assert_ok
  ...
  def test_visiting_file_custom_slug
    post('/api?token=token&slug=test', 'media' => zip)
    assert_ok
    item = App::Item.where(:slug => 'test').first
    get('/test')
  ...
  def test_visiting_file_custom_slug_type
    post('/api?token=token&slug=test', 'media' => zip)
    assert_ok
    item = App::Item.first
    get("/test.#{item.type}")
  ...
  def test_checking_file_custom_slug
    post('/api?token=token&slug=test', 'media' => zip)
    assert_ok
    item = App::Item.where(:slug => 'test').first
    authorize('user', 'pass')
  ...
  def test_deleting_file_custom_slug
    post('/api?token=token&slug=test', 'media' => zip)
    assert_ok
    item = App::Item.where(:slug => 'test').first
